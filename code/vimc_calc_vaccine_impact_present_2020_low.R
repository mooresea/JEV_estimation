###
### Calculate vaccine impact under low pop-at-risk scenario
###
library(maptools)
library(plyr)
...
###Use 10 pigs/ducks threshold
c.df$pop.lo.pct=c.df$Pct20_pd10  
...
write.table(novacc.dat,paste("results/country_runs/je_no_vaccination_",cname,"_low.csv",sep=""),sep=",",row.names = F,col.names = T)
write.table(rout.dat,paste("results/country_runs/je_routine-only_",cname,"_low.csv",sep=""),sep=",",row.names = F,col.names = T)
write.table(vacc.dat,paste("results/country_runs/je_vaccination_",cname,"_low.csv",sep=""),sep=",",row.names = F,col.names = T)
