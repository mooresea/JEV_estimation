...
###Use 2 pigs/ducks threshold
c.df$pop.lo.pct=c.df$Pct20_pd2  
...
###COMMENT OUT TO USE EXISTING STOCHASTIC SAMPLES
##
##Get posterior distribution of FOI
##
# foi.fits=read.csv("results/foi_stan_fits.csv",header=T)
# ##Use country fit if available, otherwise use sample of all model fits
# if(any(foi.fits$ctry==cname)){
#   foi.runs=sample(foi.fits$foi[foi.fits$ctry==cname],size=nruns,replace = T)
# }else{
#   foi.runs=sample(foi.fits$foi,size=nruns,replace = T)
# }


###Calculate deaths and cases
###
# beta.symp = optim(
#   par=c(2,4),
#   function(par){
#     abs(qbeta(0.01,par[1],par[2]) - 0.1) +
#       abs(qbeta(0.25,par[1],par[2]) - 0.2) +
#       abs(qbeta(0.5,par[1],par[2]) - 0.3)+
#       abs(qbeta(0.975,par[1],par[2]) - 0.75)})
# 
# gamma.symp = optim(
#   par=c(2,4),
#   function(par){
#     abs(qgamma(0.025,par[1],par[2]) - 100) +
#       abs(qgamma(0.5,par[1],par[2]) - 300)+
#       abs(qgamma(0.975,par[1],par[2]) - 750)})
# 
# asymp.r=rgamma(10000,gamma.symp$par[1],gamma.symp$par[2])
# symp.prob=1/(1+asymp.r)
# death.prob=rbeta(10000,beta.symp$par[1],beta.symp$par[2])

##Test single run
#symp.runs=mean(symp.prob)
#death.runs=mean(death.prob)
#foi.runs=mean(foi.dist)

##
## Parameter sets
##

# ##Generate these for the first country only- then draw from file so that params are same across countries
# symp.runs=c(1/(rgamma(nruns,gamma.symp$par[1],gamma.symp$par[2])),mean(symp.prob))
# death.runs=c(rbeta(nruns,beta.symp$par[1],beta.symp$par[2]),mean(death.prob))
# param.df=data.frame(run_id=1:(nruns),symptomatic_probability=symp.prob,mortality_probability=death.prob)
# write.table(param.df,"results/stochastic_template_params.csv",sep=",",row.names = F,col.names = T)
...
#param.df$foi=foi.runs
foi.runs=param.df[,which(names(param.df)==paste0("foi.",cname))]
#names(param.df)[ncol(param.df)]=paste("foi:",cname,sep="")
#write.table(param.df,"results/stochastic_template_params.csv",sep=",",row.names = F,col.names = T)
...
write.table(novacc.dat,paste("results/country_runs/je_no_vaccination_",cname,".csv",sep=""),sep=",",row.names = F,col.names = T)
write.table(rout.dat,paste("results/country_runs/je_routine-only_",cname,".csv",sep=""),sep=",",row.names = F,col.names = T)
write.table(vacc.dat,paste("results/country_runs/je_vaccination_",cname,".csv",sep=""),sep=",",row.names = F,col.names = T)
